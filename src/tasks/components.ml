...
type component = {
    domain : vtype list;
    codomain : vtype;
    apply : Value.t list -> Value.t;
    name : string;
}

(* WARNING : HERE THERE BE A LONG LIST OF COMPONENTS *)
...
module StringComponents = struct
    ...
    let str_length = {
        ...
        codomain = TInt;
        apply = (function
                    | [VString x] -> VInt (CCString.length x)
                    | _ -> VError);
        name = "length"
    }
    let str_upper = {
        ...
        codomain = TString;
        apply = (function
                    | [VString s] -> VString (CCString.uppercase_ascii s)
                    | _ -> VError);
        name = "to_upper"
    }
    ...
    let str_lower = {
        ...
        codomain = TString;
        apply = (function
                    | [VString s] -> VString (CCString.lowercase_ascii s)
                    | _ -> VError);
        name = "to_lower"
    }
    ...
    let index str sub =
        let rec go n =
            if n > (CCString.length str) - (CCString.length sub) then (-1)
            else if CCString.sub str n (CCString.length sub) = sub then n
            else go (n + 1)
        in go 0

    let split str delim = let dl = CCString.length delim in
        let rec go str = let first = index str delim in
            if first >= 0 then
                (CCString.sub str 0 first)::(go (CCString.sub str (first + dl) ((CCString.length str) - first - dl)))
            else [str]
        in go str
    ...
    let str_trim = {
        ...
        codomain = TString;
        apply = (function
            | [VString s] -> VString (CCString.trim s)
            | _ -> VError);
        name = "trim"
    }
    ...
    let reverse str =
        ...
        let res = CCString.create len in
...
let eq_longest_string = {
    ...
    codomain = TString;
    apply = (function
        | [VString l; VString r] -> if (CCString.length l) > (CCString.length r) then
